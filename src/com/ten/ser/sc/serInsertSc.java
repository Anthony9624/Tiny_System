...
/******************************************************************************/
		...
		try{
			...
			float scredit = stu2.getScredit();//学生已经选到的学科
			int sno = stu.getSNo();
			if(choiceR.length!=0){//空异常是什么鬼啊啊啊
				...
				for(int i =0;i<choiceR.length;i++){
					...
					sc[i].setClassroom("0");//上课科目应该如何录入？？？默认为0，表示没有没有科目
				...
				if(scredit<=Student.MAXscore){
					...
					if(sum!=0) {
						out.println("<h2><center>报名成功</cneter></h2>");
						response.sendRedirect("../stu/showCou.jsp");
					}
				}else {
					out.println("<h2><center>你选择的课程超过上限"+Student.MAXscore+"分！</cneter></h2>");
					response.setHeader("refresh", "2;url=../stu/baoming.jsp");
				}
			}else {
				out.println("<h2><center>报名失败</cneter></h2>");
				response.setHeader("refresh", "2;url=../stu/baoming.jsp");
			}
		}catch(Exception e){e.printStackTrace();out.println("选课失败!");response.setHeader("refresh", "2;url=../stu/baoming.jsp");}
		out.println("  </BODY>");
		out.println("</HTML>");}
		out.flush();
		out.close();
